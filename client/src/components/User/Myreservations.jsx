...
function Myreservations() {
    ...
    const fetchreservations = async () => {
        try {
            ...
            const response = await fetch(`https://easlycars-server.vercel.app/api/user/myreservations?${queryParams}`, {
                method: 'GET',
                headers: {
    ...
    const handleRemoveRese = async (id) => {
        try {
            const response = await fetch(`https://easlycars-server.vercel.app/api/reservation/delete/${id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('T_ID_Auth')}`
